...
vector<uint64_t> measure_time_dense_dnnl(const vector<float> &A, const vector<float> &B, const size_t M, const size_t K, const size_t N, float* C, const size_t n_run){
    vector<uint64_t> elapsed_times;
    float alpha = 1.0;
    float beta = .0;
    //warmup
    dnnl_sgemm('N', 'N', M, N, K, alpha, A.data(), K, B.data(), N ,beta, C, N);
...
int main(int argc, char**argv) {
    ...
    vector<uint64_t> elapsed_times = measure_time_dense_dnnl(A, B, M, K, N, C.data(), n_run);
    std::string method_name = "OneDNN";
